...
function t776_removeSizeStyles(styleStr) {
   if (styleStr && (styleStr.indexOf('font-size') !== -1 || styleStr.indexOf('padding-top') !== -1 || styleStr.indexOf('padding-bottom') !== -1)) {
      var styleStrSplitted = styleStr.split(';');
      styleStr = '';
      for (var i = 0; i < styleStrSplitted.length; i++) {
         if (styleStrSplitted[i].indexOf('font-size') !== -1 || styleStrSplitted[i].indexOf('padding-top') !== -1 || styleStrSplitted[i].indexOf('padding-bottom') !== -1) {
...
function t776_copyTypography(recid) {
   var rec = document.getElementById('rec' + recid);
   if (!rec) return;
   var productTitle = rec.querySelector('.t776__title');
   var productDescription = rec.querySelector('.t776__descr');
   if (!productTitle || !productDescription) {
      return
   }
   var titleStyle = productTitle.getAttribute('style');
   var descriptionStyle = productDescription.getAttribute('style');
   var popupTitles = rec.querySelectorAll('.t-popup .t776__title');
   Array.prototype.forEach.call(popupTitles, function (title) {
      title.setAttribute('style', t776_removeSizeStyles(titleStyle))
   });
   var popupDescription = rec.querySelectorAll('.t-popup .t776__descr, .t-popup .t776__text');
   Array.prototype.forEach.call(popupDescription, function (description) {
      description.setAttribute('style', t776_removeSizeStyles(descriptionStyle))
   })
}
