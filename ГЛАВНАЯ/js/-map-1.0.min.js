function t_appendGoogleMap(recid, key) {
   if ("object" == typeof google && "object" == typeof google.maps) t_handleGoogleApiReady(recid);
      ...
      MapElement && (mapLang = MapElLang || ""), mapLang && 2 === mapLang.length && (langPreferences = "&language=" + mapLang);
...
function t_handleGoogleApiReady(recid) {
   ...
   Array.prototype.forEach.call(gmaps, (function (gmap) {
      ...
      mapStyleAttr && (mapstyle = eval(mapStyleAttr));
      ...
      if (Array.prototype.forEach.call(arMarkers, (function (el) {
            myLatLng = new google.maps.LatLng(parseFloat(el.lat), parseFloat(el.lng)), marker = new google.maps.Marker({
               position: myLatLng,
               map: map,
               title: el.title
            }), bounds.extend(myLatLng), el.descr ? t_map__attachInfoMessage(marker, el.descr) : t_map__attachInfoMessage(marker, el.title)
         })), arMarkers.length > 1) {
...
function t_appendYandexMap(recid, key) {
   if ("object" == typeof ymaps && "function" == typeof ymaps.Map) t_handleYandexApiReady(recid);
      ...
      if (MapElement) switch (MapElAttr) {
         case "EN":
            mapLang = "en_US";
            break;
         default:
...
function t_handleYandexApiReady(recid) {
   ...
   Array.prototype.forEach.call(ymapsArr, (function (ymap) {
      ...
      Array.prototype.forEach.call(Object.keys(mobilePanelStyles), (function (name) {
         ...
         textarea.innerHTML = marker.descr, myGroup.add(new ymaps.Placemark(myLatlng, {
      ...
      arMarkers.length > 1 ? (map.setBounds(myGroup.getBounds(), {
         checkZoomRange: !0
      }).then((function () {
         ...
         map.container.fitToViewport(), arMarkers.length > 1 ? map.setBounds(myGroup.getBounds(), {
            checkZoomRange: !0
         }).then((function () {
            zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom)
         })) : zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom)
