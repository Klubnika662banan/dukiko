function t_sldsInit(rec, sliderOptions) {
 ...
 function t_slds__initFeedsSlider(rec, sliderRec) {
    var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"),
       sliderItems = sliderRec.querySelectorAll(".t-slds__item:not(.t-slds__item_dummy)"),
       totalSlides = sliderWrapper.getAttribute("data-slider-totalslides"),
       firstSlide = sliderItems[0],
       lastSlide = sliderItems[sliderItems.length - 1],
       sliderWithCycle = sliderWrapper.getAttribute("data-slider-with-cycle");
 ...
 function t_slds__hideMobileSlides(activeSlide) {
    if (activeSlide) {
       var slideWrapper = activeSlide.closest(".t-slds__items-wrapper"),
          slides = Array.prototype.slice.call(slideWrapper.querySelectorAll('.t-slds__item:not(.t-slds__item_dummy):not([aria-hidden="true"])')),
 ...
 function t_slds_updateFeedsSliderOnResize(rec, sliderRec, sliderOptions) {
    var sliderWrapper = sliderRec.querySelector(".t-slds__items-wrapper"),
       isFeedsSlider;
    sliderWrapper.classList.contains("js-feed-container") && (window.innerWidth > 960 ? "true" === sliderWrapper.getAttribute("data-feeds-posts-loaded") ? (t_slds__initFeedsSlider(rec, sliderRec, sliderOptions), t_slds_initSliderControls(rec, sliderOptions)) : document.addEventListener("feedsLoadPosts", (function () {
       t_slds__initFeedsSlider(rec, sliderRec, sliderOptions), t_slds_initSliderControls(rec, sliderOptions)
 ...
 function t_slds_animate(timing, draw, duration) {
    ...
    requestAnimationFrame((function t_slds_animate(time) {
       ...
       draw(progress), timeFraction < 1 ? requestAnimationFrame(t_slds_animate) : "y" !== window.lazy && "yes" !== document.querySelector("#allrecords").getAttribute("data--lazy") || t_slds_onHammerLoad("t_lazyload_update", (function () {
          t_lazyload_update()
       }))
    }))
 }
 ...
 function t_slds_ActiveBullet(rec, position, totalSlides, sliderOptions) {
    ...
    if (sliderRec) {
       ...
       })), sliderOptions && sliderOptions.thumbsbulletGallery && position >= maxThumbsCount && position != totalSlides + 1 || totalSlides >= maxThumbsCount && 0 == position ? bulletActive = sliderRec.querySelector('.t-slds__bullet[data-slide-bullet-for="' + maxThumbsCount + '"]') : 0 == position ? bulletActive = sliderRec.querySelector('.t-slds__bullet[data-slide-bullet-for="' + totalSlides + '"]') : position == totalSlides + 1 && (bulletActive = sliderRec.querySelector('.t-slds__bullet[data-slide-bullet-for="1"]')), bulletActive && bulletActive.classList.add("t-slds__bullet_active")
    }
 }
 ...
 function t_slds_ActiveSlide(rec, position, totalSlides) {
    ...
    if (sliderRec) {
       ...
       if (Array.prototype.forEach.call(iframes, (function (iframe) {
          ...
          })), 0 == position && !1 === sliderNotAnimated) sliderRec.querySelector('.t-slds__item[data-slide-index="' + totalSlides + '"]').classList.add("t-slds__item_active");
          ...
          firstSlide && firstSlide.classList.add("t-slds__item_active")
       } else position == totalSlides + 1 && !0 === sliderNotAnimated && (slideActive = sliderRec.querySelector('.t-slds__item[data-slide-index="1"]'));
       slideActive && slideActive.classList.add("t-slds__item_active"), window.t_slds__isiOS && t_slds__hideMobileSlides(slideActive), t_slds__setTabindexForFocusableElements(rec)
    }
 }
 ...
 function t_slds_changeImageUrl(rec) {
    var allRec = document.getElementById("allrecords"),
       isLazyActive = allRec && "yes" === allRec.getAttribute("data--lazy"),
