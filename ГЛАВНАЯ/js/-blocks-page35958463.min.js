...
function t702_initPopup(recId) {
   ...
   if (popupTooltipHook) {
      ...
      document.addEventListener('click', function (event) {
         ...
         t702__lazyLoad();
         if (analitics && window) {
            sendEventToStatistics(analitics, popupTooltipHook)
         }
      });
   ...
   popup.addEventListener('modal:show' + popupTooltipHook, function () {
      document.addEventListener('keydown', t702_escClosePopup)
   });
   popup.addEventListener('modal:close' + popupTooltipHook, function () {
...
function t702_showPopup(recId) {
   ...
   t702__lazyLoad();
   t702__triggerEvent(popup, 'modal:show' + popupTooltipHook)
}
...
function t702_closePopup(recId) {
   ...
   t702__triggerEvent(popup, 'modal:close' + popupTooltipHook)
}
...
function t702_sendPopupEventToStatistics(popupName) {
   ...
   virtPage += popupName;
   virtTitle += popupName;
   if (window && typeof sendEventToStatistics == 'function') {
      sendEventToStatistics(virtPage, virtTitle, '', 0)
   } else {
      if (ga) {
...
function t702__lazyLoad() {
   if (window.lazy === 'y' || document.getElementById('allrecords').getAttribute('data--lazy') === 'yes') {
